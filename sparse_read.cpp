...
static int64_t COPY_BUF_SIZE = 1024 * 1024;
static char *copybuf;

#if 0
static void ErrorString(int err)
{
    if (err == -EOVERFLOW) {
        printf("EOF while reading file ");
    } else if (err == -EINVAL) {
        printf("Invalid sparse file format ");
    } else if (err == -ENOMEM) {
        printf("Failed allocation while reading file ");
    } else {
...
static void verbose_error(bool verbose, int err, const char *fmt, ...)
{
    ...
    if (fmt) {
        ...
        va_list argp;
        va_start(argp, fmt);
        int printed = vsprintf(printf_buf, fmt, argp);
        va_end(argp);
        puts(printf_buf);
    }
#endif
...
struct sparse_file *sparse_file_import(int fd, int offset, bool verbose, bool crc)
{
	int ret;
	sparse_header_t sparse_header;
	int64_t len;
	...
	if (sparse_header.magic != SPARSE_HEADER_MAGIC) {
            printf("sparse_header.magic(0x%08x) != 0x%08x\n", sparse_header.magic, SPARSE_HEADER_MAGIC);
...
struct sparse_file *sparse_file_import_auto(int fd, bool crc, bool verbose)
{
	...
	s = sparse_file_import(fd, 0, verbose, crc);
	if (s) {
		return s;
	}
