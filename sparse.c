...
static int write_all_blocks_with_progress_cb(struct sparse_file *s,
        struct output_file *out, int (*cbNotifyFunction)(int64_t *pwriteLen, int64_t writeLen), int64_t *pwriteLen)
{
    struct backed_block *bb;
    unsigned int last_block = 0;
    int64_t pad;
    ...
    for (bb = backed_block_iter_new(s->backed_block_list); bb;
        ...
        last_block = backed_block_block(bb) +
            DIV_ROUND_UP(backed_block_len(bb), s->block_size);
        /* For Status updation. Not thread safe. */
        if (cbNotifyFunction && pwriteLen) {
            int64_t writeLen = (int64_t)last_block * s->block_size;
            cbNotifyFunction(pwriteLen, writeLen);
        }
    }
...
int sparse_file_write_with_progress_cb(struct sparse_file *s, int fd, bool gz, bool sparse,
        bool crc, int (*cbNotifyFunction)(int64_t *pwriteLen, int64_t writeLen), int64_t *pBuffer)
{
    int ret;
    int chunks;
    struct output_file *out;
    ...
    ret = write_all_blocks_with_progress_cb(s, out, cbNotifyFunction, pBuffer);
